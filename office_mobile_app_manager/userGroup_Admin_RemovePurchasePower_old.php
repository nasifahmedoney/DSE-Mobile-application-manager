...
$success_count = 0;
if(isset($_POST['inv_codes_old']))
{
	if($user_email != '')
	{
		$codes_to_approve = $_POST['inv_codes_old'];
		...
		for($x = 0; $x < $total; $x++)
		{
			...
			$query = "UPDATE old_documents SET purchase_power = '$purchase_power' WHERE investor_code = '$codes'";
		...
		if($success_count == $total)
		{
			echo '<script type="text/javascript"> 
				alert("Purchase power removed successfully.");
...
<a class="navbar-brand" href="userGroup_Admin.php" ><?php echo before('@',$user_email); ?></a>
    ...
    <ul class="navbar-nav">
	  ...
	  <li class="nav-item">
        <a class="nav-link" href="#"><b>Remove Purchase Power(Old)</b></a>
      </li>
...
<?php echo '<b>Remove Mobile Application Codes From Old Database</b>';echo '<br>';echo '<p></p>'; ?>

<form id = "form" action = "userGroup_Admin_RemovePurchasePower_old.php" method = "POST">
...
$query = "SELECT * FROM old_documents WHERE purchase_power ='set_90_percent'";
$result = mysqli_query($link,$query);
...
if($result)
{
    while($row = $result->fetch_array(MYSQLI_ASSOC))
    {
        if(is_array($row))
        {
            ...
            <tbody>
                <tr>
                    <td><input type="hidden" name="inv_codes_old[]" value="<?php echo $row['investor_code']; ?>"><?php echo $row['investor_code']; ?></td>
                </tr>
            </tbody>
